...
## Language Rules (Windows cp932 encoding workaround)
- **All queries to NotebookLM (`notebooklm ask`, source titles, notebook names, research queries) MUST be written in English.** Japanese input causes UnicodeEncodeError (cp932) in the terminal output.
- **After retrieving results, translate the English output into Japanese** before presenting to the user or writing to Obsidian notes.
- When saving responses to a temp file, always redirect output: `notebooklm ask "..." --json > C:/temp/output.json`

## ⚠️ Windows: `notebooklm ask` の安全なクエリパターン（必須）

**plain text の `notebooklm ask "query"` は Windows で cp932 クラッシュを起こす場合がある。**

英語の回答には em-dash `—`（U+2014）等の非cp932文字が頻出し、ターミナルの表示処理がクラッシュする。また `--json` フラグは `cited_text` を含む 200〜600KB のファイルを生成し、Read ツールの上限（256KB）を超える。

**唯一の安全パターン（すべての `ask` で必須）：**

```bash
# ① --json でファイルに書き出す
notebooklm ask "query" --json > C:/temp/output.json

# ② answer フィールドだけを UTF-8 ファイルに抽出（cp932 エラー回避 + ファイルサイズ削減）
python -c "
import json
...
> ⚠️ `output.json` を直接 Read ツールで読もうとしてはならない。必ず `output_answer.txt` 経由で取得すること。

**クエリ長の指針：** 1クエリは英語で 80字以内を目安にする。長すぎるとタイムアウトが発生する。必要であればクエリを複数回に分割すること。

## ⚠️ Windows: 重複ソースの一括削除

`notebooklm source delete <id>` はデフォルトで y/N の対話プロンプトを出す。そのため bash ループでの一括削除は途中で止まる。**Python subprocess + `--yes` フラグ** が唯一の確実な方法。
...
# ③ 重複分（1件残して余分を削除）を --yes で一括削除
seen = set()
for s in sources:
    t = s['title']
    if t in dupe_titles:
        if t in seen:
            subprocess.run(['notebooklm', 'source', 'delete', s['id'], '--yes'])
        else:
            seen.add(t)
```

## ⚠️ Windows cp932: source add-research MUST use two-step pattern

**NEVER run `notebooklm source add-research "query"` without `--no-wait`.**

The display step after research completion crashes with `UnicodeEncodeError: cp932` when source titles contain em dashes (`—`) or other non-cp932 characters.

**⚠️ cp932クラッシュの実際の挙動（重要）:**  
クラッシュは「表示処理」で発生するため、インポート処理自体は実行済みの場合が多い。さらに `research wait --import-all` を再実行したり、前回クラッシュ済みのタスクに対して再度実行すると、同じソースが複数回インポートされ**大量の重複（例：10ソース×5回=50件）が発生する**。クラッシュ後は必ず `source list --json` で件数・重複を確認してから次の手順に進むこと。

**Standard pattern: one specific query, then import all results.**

```bash
# Step 1: Start research (non-blocking, no display)
# ⚠️ source add-research に --json オプションは存在しない。--no-wait のみ使用すること
notebooklm source add-research "query" --no-wait

# Step 2: Wait and import ALL results from that query — 1回だけ実行すること
# ⚠️ Windows では /c/temp/ ではなく C:/temp/ を使用すること
# ⚠️ exit code 1（失敗）でも絶対に再実行しないこと → 重複インポートの根本原因
notebooklm research wait --import-all --json > C:/temp/research_import.json

# Step 2 が exit code 1 で失敗した場合のフォールバック（再実行NG）:
#   ① まず source list --json で部分インポート済み件数を確認する
#   ② research status --json で見つかったURLと比較して未インポート分だけを特定する
#   ③ 未インポートのURLのみ notebooklm source add "<url>" で手動追加する
#   ④ ② で確認済みのURLのみを追加すること（自力でURLを生成・推測することは禁止）

# Step 3: ⚠️ MANDATORY — check for duplicates immediately after every import
# research wait --import-all may internally retry on timeout, importing the same sources multiple times
python -c "
import subprocess, json
from collections import Counter
r = subprocess.run(['notebooklm', 'source', 'list', '--json'], capture_output=True, text=True, encoding='utf-8')
sources = json.loads(r.stdout).get('sources', [])
titles = [s['title'] for s in sources]
dupes = {t: c for t, c in Counter(titles).items() if c > 1}
print(f'Total sources: {len(sources)}')
if dupes:
    print('WARNING: Duplicates found:', dupes)
    print('Run the bulk-delete script in the Windows duplicate removal section above.')
else:
...
> ⚠️ **重複の根本原因は2つある:**
> 1. `research wait --import-all` はタイムアウト時に内部リトライを行い、同じソースを複数回インポートする場合がある（例：10ソース × 6回リトライ = 60件）
> 2. `research wait --import-all` を複数回実行すると、1回目が部分インポートで失敗していても2回目で全件が再インポートされ重複が発生する
>
> **対策: `research wait --import-all` は1タスクにつき必ず1回だけ実行する。失敗してもリトライせずStep 3のチェックに進む。**

One well-targeted query returns ~10 sources and is sufficient for atomic note creation. Do NOT run multiple queries on overlapping topics.

**Alternative pattern (zero duplicate risk) — use for focused single-topic queries:**

Instead of `--import-all`, check research status and manually add only the best 2-3 sources:

```bash
# Step 1: Start research
notebooklm source add-research "query" --no-wait

# Step 2: Check what was found (do NOT run research wait --import-all)
notebooklm research status --json > C:/temp/research_status.json

# Step 3: Read status.json, pick top URLs, add manually (zero duplicate risk)
python -c "
import json
with open('C:/temp/research_status.json', encoding='utf-8') as f:
    d = json.load(f)
tasks = d.get('tasks', d.get('research_tasks', []))
if tasks:
    sources = tasks[0].get('sources', tasks[0].get('found_sources', []))
    for s in sources[:5]:
        print(s.get('url', s.get('uri', s)))
"
# Then: notebooklm source add "https://chosen-url.com" --json 2>/dev/null
```

この方法は重複リスクがゼロだが、URLを手動で選別する手間がかかる。公式プレスリリース等、URLが予測可能な場合に特に有効。

This applies to **both fast and deep mode**. The `--no-wait` flag separates display from import, making the process crash-safe.

**If research was already run without `--no-wait` and sources were not imported:**
```bash
# Check status of completed-but-not-imported research
notebooklm research status --json > C:/temp/research_status.json

# Import all completed research
notebooklm research wait --import-all --json > C:/temp/research_import.json
```

**⚠️ `research wait --import-all` imports only the most recent active task, NOT all historical completed tasks.**

**Prefer one specific query over multiple broad queries.** NotebookLM uses RAG internally, so adding irrelevant sources increases token cost without improving answer quality. A single well-targeted query (10 sources) outperforms two overlapping queries (20 sources).

If multiple `source add-research` calls were made, only the latest task is imported. Earlier tasks must be imported manually:
```bash
# Check how many sources are in the notebook vs how many research found
notebooklm source list --json > C:/temp/sources.json        # count imported
notebooklm research status --json > C:/temp/status.json     # count per task

# If counts differ, add missing URLs from research status output manually:
notebooklm source add "https://missing-url.com" --json 2>/dev/null
```
...
| Web research (fast) | `notebooklm source add-research "query" --no-wait --json > /c/temp/r.json` |
| Web research (deep) | `notebooklm source add-research "query" --mode deep --no-wait --json > /c/temp/r.json` |
| Check research status | `notebooklm research status --json > /c/temp/status.json` |
| Wait for research + import | `notebooklm research wait --import-all --json > /c/temp/import.json` |
...
**Failed source cleanup:** After waiting, run `notebooklm source list --json` and check for `"status": "error"` entries. Delete them immediately with `notebooklm source delete <id>` — do not wait or retry these. Proceed with the remaining ready sources.
...
**⚠️ Mode confirmation rule:** If the calling skill or user instruction does not explicitly specify fast or deep, **ask the user before running research**:
```
リサーチモードを選択してください：
- fast（速い・5〜10件）: 特定トピックの概要把握向け
- deep（詳細・20件以上、2〜5分）: 広範なトピックの網羅的分析向け
```
Do not default silently to fast mode when context is ambiguous.
...
| Source `status: error` after processing | Access denied / 404 / paywall | Run `notebooklm source list --json`, delete all error sources with `notebooklm source delete <id>`, proceed with remaining ready sources — do NOT retry |
| Source fails to add (domain restriction) | Site blocks NotebookLM crawler | Prepend `https://pure.md/` to the URL: `notebooklm source add "https://pure.md/https://example.com/page"`. Verified effective for Yahoo Finance, Nikkei. Does NOT bypass paywalls (e.g. Investopedia fails regardless). Try once; if still fails, skip the source. |
| `source wait` exits with code 1 | Source failed to process | Delete the failed source immediately and proceed without it — do NOT retry indefinitely |
| `research wait --import-all` exits with code 1 | Bulk import server timeout or partial failure | ⚠️ 再実行禁止。① `source list --json` で部分インポート済み件数を確認 → ② `research status --json` で未インポートURLを特定 → ③ そのURLのみ `source add` で手動追加。`research wait --import-all` を再実行すると重複インポートが発生する |
| `UnicodeEncodeError: cp932` on `source add-research` | Source titles contain em dash `—` or other non-cp932 chars; display crashes before import runs | Always use `--no-wait --json > /c/temp/r.json` then `research wait --import-all --json > /c/temp/import.json`. If already crashed: run `research wait --import-all --json > /c/temp/import.json` to recover |
